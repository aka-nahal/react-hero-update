import {PeakHeroComponents} from './peakhero-components'
import Http from './http'

export function PeakHeroPopups () {
  let iframeWrap, iframe, iDoc, peakComponents, currentPopupId
  ...
  function addIframe () {
    ...
    iframeWrap.innerHTML =
      `<iframe id="peak-hero-iframe" allowtransparency="true" style="display: none"></iframe>
       <style>
         iframe#peak-hero-iframe {
  ...
  function showPopup (popup) {
    ...
    popupContent.innerHTML = popup.popup_template.html
    popupContent.style.backgroundColor = popup.settings.background_color
    if (popup.settings.background_image && popup.settings.background_image.url) {
      popupContent.style.backgroundSize = popup.settings.background_size || 'initial'
      ...
      popupContent.style.backgroundImage =
        `url(//${__DEV__ ? 'localhost:3000' : 'converthero.com'}/${popup.settings.background_image.url})`
    } else {
      popupContent.style.backgroundSize = 'initial'
      popupContent.style.backgroundImage = 'none'
    }
    iDoc.getElementById('styles').innerHTML =
      `<style>${popup.css}</style>` +
      (popup.color_schema ? '<style>' + popup.color_schema.css + '</style>' : '') +
      `<style>${popup.css_override}</style>`
    peakComponents = PeakHeroComponents(iDoc, popup.data, popup.settings, closePopup, {popupId: popup.id})
    iDoc.getElementById('popup-close').onclick = closePopup
    iDoc.getElementById('popup-animate-container').className = 'animated ' + popup.settings.appear_animation
    iDoc.getElementById('backdrop').style.opacity = popup.settings.backdrop_opacity
  ...
  return {
    showPopup: (popup) => {
      showPopup(popup)
      Http.post('api/impressions', {id: popup.id})
        .catch((error) => {
          console.warn('Unable to report popup show:', error)
        })
    },
    closeActivePopup: () => {
      closePopup()
