...
export function PopupViewer () {
  ...
  const initialize = () => {
    ...
    iframeWrap.innerHTML =
      `<iframe id="peak-hero-iframe-viewer" allowtransparency="true" style="display: none;"></iframe>
       <style>
         iframe#peak-hero-iframe-viewer {
  ...
  const showPopup = (popupTemplate) => {
    ...
    if (popupTemplate.default_settings.background_image) {
    ...
    iDoc.getElementById('styles').innerHTML =
      ...
      (popupTemplate.color_schemas.length ? '<style>' + popupTemplate.color_schemas[0].css + '</style>' : '')
